...
void gramSchmidt(std::vector<std::vector<double>> &a, std::vector<std::vector<double>> &q, std::vector<std::vector<double>> &r, const size_t m, const size_t n) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        // r_ii = ||a_i||
        r[i][i] = std::sqrt(std::transform_reduce(a[i].begin(), a[i].end(), 0.0, std::plus<double>(), [](double x) { return x * x; }));
        ...
        if (r[i][i] > tol) {
            ...
            for (size_t j = 1; j < i; ++j) {
                std::transform(a[j].begin(), a[j].end(), a[i].begin(), a[i].begin(), [&](double x, double y) { return y - x * a[j][i]; });
            }
            anorm = std::sqrt(std::transform_reduce(a[i].begin(), a[i].end(), 0.0, std::plus<double>(), [](double x) { return x * x; }));            
            std::transform(a[i].begin(), a[i].end(), a[i].begin(), [&](double x) { return x / anorm; });
        }

        for (size_t j = i + 1; j < n; ++j) {
            // Compute the inner product of the ith and jth columns
            // r_ij = a_i * a_j
            r[j][i] = std::transform_reduce(a[i].begin(), a[i].end(), a[j].begin(), 0.0, std::plus<double>(), [&](double x, double y) { return x * y; });

            // a_j -= r_ij * a_i
            std::transform(a[i].begin(), a[i].end(), a[j].begin(), a[j].begin(), [&](double xi, double yi) { return yi - r[j][i] * xi; });
        }
    }
}
