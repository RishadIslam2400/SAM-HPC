...
struct ruge_stuben {
    ...
    struct params {
        ...
        /**
         * Given a threshold value 0 < eps_strong <= 1, the variable i strongly depends
         * on the variable j if, -A(i, j) >= eps_strong * max_{k != i}(-A(i, k)).
         * In practice, a value of eps_strong = 0.25 is usually taken.
         */
        float eps_strong;
                ...
                for (size_t i = r.begin(); i < r.end(); ++i) {
                    ...
                    if (prm.do_trunc) {
                        ...
                        for (size_t j = A.m_row_pointers[i], e = A.m_row_pointers[i + 1]; j < e; ++j) {
                            ...
                            if (!S.m_vals[j] || cf[A.m_col_indices[j]] != 'C') continue;
                        ...
                        // Without truncation consider all strongly connected neighbors for interpolation
                        for (size_t j = A.m_row_pointers[i], e = A.m_row_pointers[i + 1]; j < e; ++j) {
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, n), 
            [&](const tbb::blocked_range<size_t>& r) {
                for (size_t i = r.begin(); i < r.end(); ++i) {
                    ...
                    // Handling F points
                    T dia = T(0);                 // stores the values of the diagonal elements and weakly connected neighbors(F points that are not strongly connected to node i)
                    T a_num = T(0), a_den = T(0); // a_num is the sum of all negative off-diagonal elements, a_den is the sum of all the negative off diagonals that connect to strong, coarse neighbors
                    T b_num = T(0), b_den = T(0); // b_num is the sum of all positive off-diagonal elements, b_den is the sum of all the positive off diagonals that connect to strong, coarse neighbors
                    T d_neg = T(0), d_pos = T(0); // sum of the connections that will be dropped
                    ...
                    if (prm.do_trunc) {
                        // check if the sum of the negative strong connections that are being kept is significant
                        if (std::abs(a_den - d_neg) > epsilon) {
                            // ratio of the total negative strong connections and the strong connections being kept
                    ...
                    // positive off diagonal elements that are very small are included in the digonal sum
                    if (T(0) < b_num && std::abs(b_den) < epsilon) {
                        dia += b_num;
                    }

                    // final interpoaltion coefficients: -(sum of all connections) / (diagonal * sum of all strong connections)
                    T alpha = std::abs(a_den) > epsilon ? -cf_neg * std::abs(a_num) / (std::abs(dia) * std::abs(a_den)) : 0;
                    T beta = std::abs(b_den) > epsilon ? -cf_pos * std::abs(b_num) / (std::abs(dia) * std::abs(b_den)) : 0;
...
private:
    ...
    ///
    /// @details Analyzes the system matrix `A` to build a boolean "strength of connection"
    /// matrix `S` based on the fact that if i strongly depends on j, then j strongly influnces i.
    /// It also identifies nodes with no strong off-diagonal influence
    /// and pre-classifies them as 'F' (Fine) points.
    ///
    /// On return:
    ...
    static void connect(const CSRMatrix<T>& A, float eps_strong, CSRMatrix<char>& S, std::vector<char>& cf) {
        const size_t n = A.m_rows;
        ...
        tbb::parallel_for(tbb::blocked_range<size_t>(0, n),
            [&](const tbb::blocked_range<size_t>& r) {
                for (size_t i = r.begin(); i < r.end(); ++i) {
                    ...
                    // If there are no negative off-diagonals, this point is not strongly
                    // coupled to any other node and cannot be used for interpolation. 
                    ...
                    // Iterate through the row again and mark all connections that are
                    // stronger than the threshold. This identifies the set of points that strongly
                    // influnence i (i strongly depends on) and can be used to interpolate i.
                    for (size_t j = A.m_row_pointers[i], e = A.m_row_pointers[i + 1]; j < e; ++j) {
        ...
        for (size_t i = 0; i < nnz; ++i) {
            if (S.m_vals[i])
                ++(S.m_row_pointers[A.m_col_indices[i] + 1]);
        }
        ...
        // 2e. Finalize the row pointers. The previous step left them pointing to the
        // end of each row's data. This trick restores them to point to the beginning.
        std::rotate(S.m_row_pointers.data(), S.m_row_pointers.data() + n, S.m_row_pointers.data() + n + 1);
        S.m_row_pointers[0] = 0;
    }

    /// @brief Splits the variables into Coarse (C) and Fine (F) sets based on strength of connection.
    /// @details The two heuristic for C/F splitting:
    /// 1. For each F-point i, every point j in S[i] that strongly influences i either should
    /// be in the coarse interpolatory set C_i or should strongly depend on at least one point
    /// in C_i.
    /// 2. The set of coarse points C should be a maximal subset of all points with the
    ...
    /// @param S The transposed strength matrix S^T. Used to find nodes that strongly influence a given node.
    /// @param cf [in,out] The coarse/fine splitting vector.
    static void cfsplit(const CSRMatrix<T>& A, const CSRMatrix<char>& S, std::vector<char>& cf) {
        ...
        // `ptr[k]` will be the starting index in the sorted `i2n` array for nodes with lambda value k.
        std::vector<size_t> ptr(n + 1, 0);
        // `cnt[k]` is a running counter for the number of nodes placed in the bucket for lambda value k.
        ...
        // 2. The node selected as a C-variable should appear in the interpolation formula
        // for the nodes that strongly depend on this point (the correspnding row in S^T).
        // So they should become F-variables.
        ...
        for (size_t top = n; top-- > 0;) {
            ...
            // If the highest priority remaining is 0, it means all remaining 'U' nodes
            // are not strongly influenced by any other 'U' nodes. We can safely make them
            // all C-points to ensure the coarse grid covers the entire domain.
            if (lam == 0) {
            ...
            // If this node has already been made an F-point by a previously chosen C-point,
            // there is nothing to do, so we skip it.
            if (cf[i] == 'F') continue;
            ...
            // Since the neighbors in C no longer needs to influence C their lambnda value is decreased
            // This maintains H-2
            for(size_t j = A.m_row_pointers[i], e = A.m_row_pointers[i + 1]; j < e; j++) {
                if (!S.m_vals[j]) continue;
