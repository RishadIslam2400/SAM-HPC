...
/* Given two vectors of the same length and an index this function returns
  ...
  Example: Suppose x is a vector {1, 2, 3, 4}, y is a vector {5, 6, 7, 8} and
  index = 1 Then the value returned by partialdot_product(x, y) is 65, which is
  computed by x[1] * y[1] + x[2] * y[2] + x[3] * y[3] = 2 * 6 + 3 * 7 + 4 * 8
        = 12 + 21 + 32
        = 65

  Features: This implementation has time complexity
  O(x.size()) and requires O(1) additional memory.          */
inline double partial_dot_product(const std::vector<double>& x, const std::vector<double>& y,
                                  const size_t index)
{
  assert(x.size() == y.size() && "Vectors must be of the same size.");
  assert(index <= x.size() && "Index out of bounds.");

  return std::transform_reduce(x.begin() + index, x.end(), y.begin() + index, 0.0,
...
inline void scalar_div(std::vector<double>& x, const double r) {
  std::transform(x.begin(), x.end(), x.begin(), [&r](double a) { return a / (r + 1e-13); });
}

/* -------------------- subdot_product -------------------- */
/* Given two vectors and an index this function returns the dot product of the two
  vectors x[index : index + length] and y[0 : length]. It is necessary that
  index + length is at most the length of the first array.
  ...
  is -26, which is computed by x[2] * y[0] + x[3] * y[1] + x[4] * y[2]
    = 3 * -1 + 4 * -2 + 5 * -3
    = -3 - 8 - 15
    = -26.
  ...
  Features: This implementation has time complexity
  O(length) and requires O(1) additional memory.          */
inline double subdot_product(const std::vector<double>& x,
                             const std::vector<double>& y, const size_t index)
{
  assert(index <= x.size() && "Index out of bounds.");
  assert(index + y.size() <= x.size() && "Vectors must be of the same size.");

  return std::transform_reduce(x.begin() + index, x.end(), y.begin(), 0.0,
                               std::plus<double>(), [](double a, double b) { return a * b; });
}

/* --------------------- partialscalar_sub --------------------- */
/* Given two vectors, a scalar value, and an index, this function multiplies the the
  first vector by the scalar value and then subtracts the computed components from the
  components the second vector starting at the given index.

  Input variables:
    x     : vector whose components are to be multiplied by r then subtracted from the
  ...
  Features: This implementation has time complexity
  O(length) and requires O(1) additional memory.               */
inline void partialscalar_sub(const std::vector<double>& x, const double r,
                              const size_t index, std::vector<double>& y)
{
  assert(index <= y.size() && "Index out of bounds.");
  assert(index + x.size() <= y.size() && "Vectors must be of the same size.");

  std::transform(x.begin(), x.end(), y.begin() + index, y.begin() + index,
                 [r](double a, double b) { return b - r * a; });
}

/* ----------------------- householder ----------------------- */
/*  Given a matrix A of dimension m by n (with n <= m) and arrays v_i of dimension m-i,
  for i = 1, ..., m - 1, respectively, this algorithm computes n reflection vectors
  and the factor R of a full QR decomposition of A, where R is a m by n upper triangular
  matrix. The n reflection vectors are stored in the arrays v_1, ..., v_n and the
  columns of A are overwritten by the columns of R.

  Input variables:
    a: reference to a 2d vector, the ith element of which should correspond to the ith
       column of the matrix A. During the algorithm, the columns of R will overwrite
       the columns of A.
    v: reference to vector of vectors in which the ith reflection vector of dimension m
       - i will be stored.
    m: number of rows of A.
    n: number of columns of A.

  Features: The number of flops for this implementation is
  ~ 2 * m * n^2 - (2/3) * n^3 and requires O(1) additional memory. */

inline void householderQR(std::vector<std::vector<double>>& a,
                          std::vector<std::vector<double>>& v, const size_t m, const size_t n)
{
  for (size_t i = 0; i < n; ++i) {
    // Set v[i] equal to subvector a[i][i:m]
    v[i].assign(a[i].begin() + i, a[i].end());

    /* vpartdot = ||v[i]||^2 - v[i][0] * v[i][0]; since vpartdot is unaffected by the
       change in v[i][0], storing this value prevents the need to recalculate the entire
       norm of v[i] after updating v[i][0] in the following step */
...
inline void householderQRSolve(std::vector<std::vector<double>>& A,
                               std::vector<double>& rhs, std::span<double> x,
