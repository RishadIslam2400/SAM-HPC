% load the data
data = load('top_opt_exact_maps.mat');
exact_maps = data.exactMapArray;

sequence_of_A = load("top_opt_matrices.mat");
sequence_of_A = sequence_of_A.Mats;

data = load("top_opt_approx_maps_init.mat");
...
plot(init_res_nrm, LineWidth=2, Marker="o", MarkerSize=8);
hold on;

data = load('top_opt_approx_maps_simple.mat');
approxMaps = data.approxMapArray;
...
plot(simple_res_nrm, LineWidth=2, LineStyle="--", Marker="x", MarkerSize=8);

data = load('top_opt_approx_maps_global_squared_0.1.mat');
approxMaps = data.approxMapArray;
...
data = load('top_opt_approx_maps_global_squared_0.01.mat');
approxMaps = data.approxMapArray;
...
plot(global_res_nrm_2, LineWidth=2, LineStyle="none", Marker=".", MarkerSize=8);

data = load('top_opt_approx_maps_global_squared_0.001.mat');
approxMaps = data.approxMapArray;
...
plot(global_res_nrm_3, LineWidth=2, LineStyle="none", Marker="v", MarkerSize=8);

data = load('top_opt_approx_maps_global_squared_0.0001.mat');
approxMaps = data.approxMapArray;
...
plot(global_res_nrm_4, LineWidth=2, Marker="o", MarkerSize=8, LineStyle="-.");

data = load('top_opt_approx_maps_col_squared_0.6.mat');
approxMaps = data.approxMapArray;
...
data = load('top_opt_approx_maps_col_squared_0.7.mat');
approxMaps = data.approxMapArray;
...
data = load('top_opt_approx_maps_col_squared_0.8.mat');
approxMaps = data.approxMapArray;
...
data = load('top_opt_approx_maps_lfil_squared_3.mat');
approxMaps = data.approxMapArray;
...
data = load('top_opt_approx_maps_lfil_squared_5.mat');
approxMaps = data.approxMapArray;
...
% data = load('top_opt_approx_maps_lfil_squared_7.mat');
% approxMaps = data.approxMapArray;
...
hold off;
lgd = legend("$S(A_0)$", "$S(A_k)$", "$S(A_k)$, global $thresh = 0.1$", "$S(A_k)$, global $thresh = 0.01$", "$S(A_k)$, global $thresh = 0.001$", "$S(A_k)$, global $thresh = 0.0001$", "$S(A_k)$, column threshold $\tau = 0.6$", "$S(A_k)$, column threshold $\tau = 0.7$", "$S(A_k)$, column threshold $\tau = 0.8$", "$S(A_k)$, fixed non-zero $lfil = 3$", "$S(A_k)$, fixed non-zero $lfil = 5$");
lgd.FontSize = 12;
lgd.Interpreter = "latex";
lgd.Location = "best";
lgd.NumColumns = 2;
title("Relative Residual Plot for Topology Optimization", "FontSize",16)
