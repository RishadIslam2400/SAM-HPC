...
int main() {
    ...
    readCSCFromFile(filename1, values_Q, rowIndices_Q, colPointers_Q);
    ...
    // Fill the fullMatrix from the CSC data
    for (int col = 0; col < N; col++) {
        for (int i = colPointers_A[col]; i < colPointers_A[col + 1]; i++) {
            int row = rowIndices_A[i];
            double value = values_A[i];
            fullMatrix[row][col] = value;
        }
    }
