 ...
 *
 * This function calculates the cosine (cs) and sine (sn) parameters for a Givens
 * rotation matrix. The goal is to create a rotation that, when applied to the
 * vector [dx, dy], will zero out the second component (dy).
 *
 * The implementation includes checks to maintain numerical stability, especially
 * when one of the input values is much larger than the other, to avoid overflow
