...
void testSamConstructor1()
{
    ...
    */
    ...
    CSRMatrix<int> source(3, 4, vals1, rowPointers1, colIndices1);
    CSRMatrix<int> target(source);
    ...
    /*
    assertEquals<CSRMatrix<int>>(*map.targetMatrix, target);
    assertEquals<CSRMatrix<int>>(*map.sourceMatrix, source);
    assertEquals<CSRMatrix<int>*>(map.mappingMatrix, nullptr);
